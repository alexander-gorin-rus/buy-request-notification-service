...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/notification-service'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/notification-service'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../services/notification-service && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make notification-service-restart && make nginx-restart \
                        && make notification-service-logs-without-tail \
